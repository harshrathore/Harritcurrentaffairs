...
def create_title_slide(prs, date_str, total):
...
def create_category_slide(prs, category, articles):
    ...
    for i, art in enumerate(articles[:8]):
        ...
        content = art.get("content", "")[:200].replace("\n", " ")

        add_textbox(slide, 0.8, y, 11.5, 0.5, f"{i+1}. {title}",
                    font_size=22, bold=True, color=WHITE)
        add_textbox(slide, 1.2, y + 0.45, 11, 0.6, content,
                    font_size=16, color=LIGHT_GREEN)
        y += 1.15
        if y > 6.5:
            break
...
def create_article_slide(prs, article):
    ...
    add_textbox(slide, 0.5, 0.4, 12.3, 1.2, title,
    ...
    if source:
        tags.append(source)
    add_textbox(slide, 0.5, 1.6, 12.3, 0.5, " | ".join(tags),
                font_size=16, color=LIGHT_GREEN)

    content = article.get("content", "").replace("\n", " ")
    add_textbox(slide, 0.5, 2.3, 12.3, 4.5, content[:2000],
